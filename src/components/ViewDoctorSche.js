...
const ViewDoctorSche = () => {
    const [DocSche,setDocSche] = useState([]);

	useEffect(()=>{
		loadDocSche();
	},[]);
	const loadDocSche=async()=>{
		const result=await axios.get("http://127.0.0.1:8000/api/reception/docSche");
		setDocSche(result.data);
		// console.log(DocSche);
	}
    return (
        <div>
       ...
       <tr height="600px">
           ...
           <td>	
                <table border="1">
                    ...
                    {DocSche.map((sche, index) => {
                            return (
                                <tr>
                                      <td><input type="text" name="email" value={sche.email}/></td>
                                      <td><input type="text" name="name" value={sche.name}/></td>
                                      <td><input type="text" name="spe" value={sche.speciality}/></td>
                                      <td><input type="text" name="avl" value={sche.available}/></td>
                                      <td><input type="text" name="fee" value={sche.fee}/></td>
                                   </tr>
                            )
                        })}
