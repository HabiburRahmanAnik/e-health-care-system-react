...
const UpdateSalaryForm = (props) => {
  ...
  const [updateSalaryIsValid, setUpdateSalaryIsValid] = useState(true);
  ...
  const submitHandler = (e) => {
    ...
    if(updateSalary.trim().length ===0){
      setUpdateSalaryIsValid(false);
      return;
    }
          ...
          <form onSubmit={submitHandler}>
            ...
            {!updateSalaryIsValid && (
              <label className={classes['error-text']}>
                update salary must not be empty.
              </label>
            )}
